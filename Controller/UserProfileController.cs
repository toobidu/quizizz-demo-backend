...
public class UserProfileController
{
    ...
    public async Task<ApiResponse<UserProfileDTO>> GetMyProfileAsync(int userId)
    {
        try
        {
            Console.WriteLine($"[UserProfileController] Getting profile for userId: {userId}");
            var profile = await _userProfileService.GetUserProfileAsync(userId);
            
            if (profile != null)
            {
                Console.WriteLine($"[UserProfileController] Profile found for userId: {userId}, Username: {profile.Username}");
            ...
            {
                Console.WriteLine($"[UserProfileController] No profile found for userId: {userId}");
                return ApiResponse<UserProfileDTO>.Fail("Không tìm thấy hồ sơ");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[UserProfileController] Error getting profile for userId {userId}: {ex.Message}");
            Console.WriteLine($"[UserProfileController] Stack trace: {ex.StackTrace}");
