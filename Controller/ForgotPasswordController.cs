...
public class ForgotPasswordController
{
    ...
    public async Task<ApiResponse<string>> SendOtpAsync(ForgotPasswordRequest request)
    {
        try
        {
            Console.WriteLine($"[FORGOT_PASSWORD_CONTROLLER] Received request with email: '{request?.Email ?? "null"}'");
            
            if (request == null)
            {
                Console.WriteLine("[FORGOT_PASSWORD_CONTROLLER] Request is null");
                return ApiResponse<string>.Fail("Yêu cầu không hợp lệ", 400, "INVALID_REQUEST", "/api/forgot-password/send-otp");
            }

            Console.WriteLine($"[FORGOT_PASSWORD_CONTROLLER] Email validation: {request.ValidField()}");
            
            if (!request.ValidField())
            {
                Console.WriteLine($"[FORGOT_PASSWORD_CONTROLLER] Email validation failed for: '{request.Email}'");
        ...
        {
            Console.WriteLine($"[FORGOT_PASSWORD_CONTROLLER] Exception: {ex.Message}");
    ...
    public async Task<ApiResponse<string>> VerifyOtpAsync(VerifyOtpRequest request)
    {
        ...
        {
            Console.WriteLine($"[FORGOT_PASSWORD_CONTROLLER] Verify OTP error: {ex.Message}");
    ...
    public async Task<ApiResponse<string>> ResetPasswordAsync(ResetPasswordRequest request)
    {
        ...
        {
            Console.WriteLine($"[FORGOT_PASSWORD_CONTROLLER] Reset password error: {ex.Message}");
