    ...
    public async Task<bool> HandleAsync(HttpListenerRequest request, HttpListenerResponse response)
    {
        ...
        try
        {
            ...
            if (method == "POST" && path == "/api/role-permission/assign")
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "Gán quy?n thành công", path);
                return true;
            }
            if (method == "DELETE" && path == "/api/role-permission/remove")
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "Xóa quy?n kh?i vai trò thành công", path);
                return true;
            }
            if (method == "POST" && path == "/api/role-permission/assign-multiple")
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "Gán nhi?u quy?n thành công", path);
                return true;
            }
            if (method == "DELETE" && path == "/api/role-permission/remove-multiple")
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "Xóa nhi?u quy?n kh?i vai trò thành công", path);
                return true;
            }
