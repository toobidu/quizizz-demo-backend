    ...
    public async Task<bool> HandleAsync(HttpListenerRequest request, HttpListenerResponse response)
    {
        ...
        try
        {
            if (method == "GET" && path == "/api/roles")
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "L?y danh sách vai trò thành công", path);
                return true;
            }
            if (method == "GET" && path.StartsWith("/api/roles/exists/"))
            {
                ...
                HttpResponseHelper.WriteSuccess(response, new { exists = result }, "Ki?m tra vai trò t?n t?i", path);
                return true;
            }
                ...
                int.TryParse(path.Split("/")[4], out int permissionId))
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "L?y vai trò theo quy?n thành công", path);
                return true;
            }
                ...
                int.TryParse(path.Split("/")[4], out int userId))
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "L?y vai trò theo ngu?i dùng thành công", path);
                return true;
            }
                ...
                int.TryParse(path.Split("/")[3], out int id))
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "L?y chi ti?t vai trò thành công", path);
                return true;
            }
            if (method == "POST" && path == "/api/roles")
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "T?o vai trò m?i thành công", path);
                return true;
            }
            if (method == "PUT" && path == "/api/roles")
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "C?p nh?t vai trò thành công", path);
                return true;
            }
            ...
            if (method == "POST" && path.StartsWith("/api/roles/user/") && path.EndsWith("/permissions"))
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "L?y vai trò theo quy?n và ngu?i dùng", path);
                return true;
            }
