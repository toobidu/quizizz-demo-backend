    ...
    public async Task<bool> HandleAsync(HttpListenerRequest request, HttpListenerResponse response)
    {
        ...
        try
        {
            if (method == "GET" && path == "/api/permissions")
            {
                ...
                HttpResponseHelper.WriteSuccess(response, result, "L?y danh s�ch quy?n th�nh c�ng", path);
                return true;
            }
                ...
                int.TryParse(path.Split("/")[3], out int id))
            {
                ...
                if (result != null)
                {
                    HttpResponseHelper.WriteSuccess(response, result, "L?y chi ti?t quy?n th�nh c�ng", path);
                }
                else
                {
                    HttpResponseHelper.WriteNotFound(response, "Kh�ng t�m th?y quy?n", path);
                }
                return true;
            }
