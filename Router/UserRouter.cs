...
public class UserRouter : IBaseRouter
{
    ...
    public async Task<bool> HandleAsync(HttpListenerRequest request, HttpListenerResponse response)
    {
        ...
        if (token == null)
        {
            HttpResponseHelper.WriteUnauthorized(response, "Thiếu hoặc sai token xác thực", path);
            return true;
        }
        ...
        try
        {
                ...
                int.TryParse(path.Split("/")[3], out int typeAccId))
            {
                ...
                {
                    HttpResponseHelper.WriteBadRequest(response, "Thiếu hoặc sai tham số typeAccount", path);
                    return true;
                }
                ...
                int.TryParse(path.Split("/")[3], out int getTypeId))
            {
                ...
                if (result == null)
                    HttpResponseHelper.WriteUnauthorized(response, "Không thể lấy loại tài khoản", path);
                else
                    HttpResponseHelper.WriteSuccess(response, new { typeAccount = result }, "Lấy loại tài khoản thành công", path);
                return true;
            }
            ...
            if (method == "GET" && path.StartsWith("/api/users/map-role") && request.Url?.Query != null)
            {
                ...
                {
                    HttpResponseHelper.WriteBadRequest(response, "Thiếu tham số typeAccount", path);
                    return true;
                }

                var roleId = await _userController.MapTypeAccountToRoleIdAsync(typeAccount, token);
                HttpResponseHelper.WriteSuccess(response, new { roleId }, "Ánh xạ thành công", path);
                return true;
            }
